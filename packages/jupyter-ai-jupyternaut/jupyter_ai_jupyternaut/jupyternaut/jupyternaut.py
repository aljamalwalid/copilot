...
class JupyternautPersona(BasePersona):
    """
    Default Cloudera Copilot persona (upstream class name kept for entry-point compatibility).
    """
    ...
    def defaults(self):
        return PersonaDefaults(
            ...
            avatar_path=COPILOT_AVATAR_PATH,
            description="Cloudera Copilot — the standard assistant in JupyterLab for this runtime.",
            system_prompt="...",
        )
    ...
    async def process_message(self, message: Message) -> None:
        if not hasattr(self, "config_manager"):
            self.send_message(
                "Cloudera Copilot requires the `jupyter_ai_jupyternaut` server extension package.\n\n",
        ...
        if not self.config_manager.chat_model:
            self.send_message(
                ...
                "Set one in **Settings → AI Settings** from the menu bar."
            )
            return
